...
// Build per-profile indexes, then wrap into a ProjectIndex.
var builtIndexes = new List<(string alias, SymbolIndex index)>();
var rowArtifacts = new List<RowArtifact>();
...
foreach (var kv in groups.OrderBy(k => k.Key.alias, StringComparer.Ordinal))
{
    ...
    builtIndexes.Add((alias, index));
}

// Optional exploded outputs (useful for tooling) and optional rows artifacts.
foreach (var (alias, index) in builtIndexes.OrderBy(t => t.alias, StringComparer.Ordinal))
{
    ...
    if (!string.IsNullOrWhiteSpace(emitRowsFormat))
    {
        ...
        rowArtifacts.Add(new RowArtifact { ProfileId = index.ProfileId, Format = emitRowsFormat, Path = NormalizeSlashes(rowsOutPath) });
    }
}

// Deterministic ordering: by profile_id.
var indexesSorted = builtIndexes
    .OrderBy(t => t.index.ProfileId, StringComparer.Ordinal)
    .Select(t => t.index)
    .ToList();
...
var projectIndex = new ProjectIndex
{
    ...
    GeneratedAt = commonOptions.GeneratedAt,
    EngineVersion = "2.3.0",
    RegistryId = registryPath,
    Indexes = indexesSorted,
    Artifacts = rowArtifacts.Count > 0 ? new ProjectArtifacts { Rows = rowArtifacts } : null,
    Diagnostics = new List<Diagnostic>()
};

// Always write canonical ProjectIndex to --out.
IndexEngine.WriteJson(projectIndex, outPath);
Console.WriteLine($"Wrote {outPath}");
