...
public sealed class Lexer
{
    ...
    public Regex ProfileIdentifierRegex => _idRegex;
    ...
    public IEnumerable<Token> Tokenize(string fileId, ReadOnlySpan<char> text)
    {
        ...
        while (i < text.Length)
        {
            ...
            if (state == LexState.Default)
            {
                ...
                // Punctuation tokens used by profile symbol policies (e.g. qualified identifiers).
                if (ch == ':' && i + 1 < text.Length && text[i + 1] == ':')
                {
                    int startCol = col;
                    int startByte = bytePos;
                    // consume both ':'
                    bytePos += Utf8Len(':'); i++; col++;
                    bytePos += Utf8Len(':'); i++; col++;
                    yield return new Token("Punct", "::", line, startCol, col, startByte, bytePos);
                    continue;
                }
                if (ch == '.')
                {
