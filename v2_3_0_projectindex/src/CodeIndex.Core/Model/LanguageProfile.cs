...
public sealed class LanguageProfile
{
    ...
    [JsonPropertyName("symbol_policy")] public SymbolPolicy? SymbolPolicy { get; init; }
}

public sealed class SymbolPolicy
{
    [JsonPropertyName("mode")] public required string Mode { get; init; } // all | declared

    [JsonPropertyName("exclude_single_letter_identifiers")]
    public bool ExcludeSingleLetterIdentifiers { get; init; } = false;

    [JsonPropertyName("include_qualified_identifiers")]
    public string IncludeQualifiedIdentifiers { get; init; } = "none"; // none | dot | scope | dot_and_scope

    [JsonPropertyName("include_include_headers")]
    public bool IncludeIncludeHeaders { get; init; } = false;
}
